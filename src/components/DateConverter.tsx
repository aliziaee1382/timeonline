...
export const DateConverter: React.FC = () => {
  ...
  return (
    <section id="converter" className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8">
      <div className="rounded-3xl backdrop-blur-2xl bg-slate-900/70 dark:bg-slate-900/70 light:bg-white/80 border border-violet-500/20 dark:border-violet-500/20 light:border-slate-200 shadow-2xl p-6 sm:p-8 md:p-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800/80">
          ...
          <button
            onClick={setToday}
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-medium text-slate-200 border border-slate-700 transition-all self-start sm:self-auto"
          >
        ...
        <div className="flex flex-wrap gap-2 my-6">
          ...
          >
            گاه‌شمار هجری قمری (مبدأ)
          </button>
        </div>

        {/* Input Controls */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-950/40 dark:bg-slate-950/40 light:bg-slate-100 border border-slate-800/60 mb-8">
          {sourceMode === 'jalali' && (
            <>
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">روز شمسی</label>
                <select
                  value={jDay}
                  onChange={(e) => setJDay(Number(e.target.value))}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-violet-500"
                >
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">ماه شمسی</label>
                <select
                  value={jMonth}
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">سال شمسی</label>
                <input
                  type="number"
                  value={jYear}
                  ...
                  min={1}
                  max={3000}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-violet-500 font-mono"
                />
              </div>
            </>
          )}
          ...
          {sourceMode === 'gregorian' && (
            <>
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">Day (روز میلادی)</label>
                <select
                  value={gDay}
                  onChange={(e) => setGDay(Number(e.target.value))}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-cyan-500"
                >
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">Month (ماه میلادی)</label>
                <select
                  value={gMonth}
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">Year (سال میلادی)</label>
                <input
                  type="number"
                  value={gYear}
                  ...
                  min={622}
                  max={3500}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-cyan-500 font-mono"
                />
              </div>
            </>
          )}
          ...
          {sourceMode === 'hijri' && (
            <>
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">روز قمری</label>
                <select
                  value={hDay}
                  onChange={(e) => setHDay(Number(e.target.value))}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-emerald-500"
                >
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">ماه قمری</label>
                <select
                  value={hMonth}
              ...
              <div>
                <label className="block text-xs text-slate-400 mb-1.5 font-medium">سال قمری</label>
                <input
                  type="number"
                  value={hYear}
                  ...
                  min={1}
                  max={3000}
                  className="w-full px-3 py-2.5 rounded-xl bg-slate-900 dark:bg-slate-900 light:bg-white text-slate-100 dark:text-slate-100 light:text-slate-900 border border-slate-700 text-sm outline-none focus:border-emerald-500 font-mono"
                />
              </div>
            </>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          {/* Solar Result Card */}
          <div className="relative overflow-hidden rounded-2xl p-5 bg-gradient-to-br from-violet-950/40 via-slate-900/80 to-slate-900/90 border border-violet-500/30 shadow-lg">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-violet-400">گاه‌شمار هجری شمسی</span>
              {result.isJalaliLeap ? (
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              ...
              <button
                ...
                title="کپی تاریخ"
              >
                {copiedKey === 'jalali' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          <div className="relative overflow-hidden rounded-2xl p-5 bg-gradient-to-br from-cyan-950/40 via-slate-900/80 to-slate-900/90 border border-cyan-500/30 shadow-lg">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-cyan-400">گاه‌شمار میلادی</span>
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              ...
              <button
                ...
                title="کپی تاریخ"
              >
                {copiedKey === 'gregorian' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
          ...
          {/* Lunar Result Card */}
          <div className="relative overflow-hidden rounded-2xl p-5 bg-gradient-to-br from-emerald-950/40 via-slate-900/80 to-slate-900/90 border border-emerald-500/30 shadow-lg">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-emerald-400">گاه‌شمار هجری قمری</span>
            ...
            <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <span className="flex items-center gap-1.5 text-emerald-300 font-medium">
                ...
                <span>سن متولد: {calculateAge(result.gregorian.year, result.gregorian.month, result.gregorian.day)}</span>
              </span>
              <button
                ...
                title="کپی تاریخ"
              >
                {copiedKey === 'hijri' ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
